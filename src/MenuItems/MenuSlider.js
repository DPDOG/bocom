...
import {
    ...
    View,
    TouchableOpacity,
    Image, Animated, Dimensions, FlatList, ActivityIndicator,
    Alert, I18nManager, AsyncStorage,
...
let distfromBottomAndroid, BarId = '';
console.log(height)
if (height <= 640) {
...
var discountAmount, discountPercent, taxAmount, taxPercent = '';
let allInfo = [];
let startPrice, amountWithTax = null;
let ImageWidth = width * 0.20;
let ImageHeight = width * 0.20;
let LenghtObj = [];
let AllProducts = [], MainCategory = [], SubCategory = [], SubSubCategory = [];
var total = 0, savedIDBar = 0, userCheckedIn = 0, subTotal = 0;
...
export default class MenuSlider extends React.Component {
    ...
    constructor(props) {
        ...
        AsyncStorage.getItem('LocationDiscountPercent').then((value) => {
            ...
            headerRight: (
                <TouchableOpacity onPress={() => {
                    ...
                    <Text style={{
                        ...
                        color: 'white',
                        marginRight: 5
                    }}>{localeStrings.menuSliderStrings.pay}</Text>
                </TouchableOpacity>),
            headerLeft: (
    ...
    handler() {
        this.state.TitleBehind = localeStrings.menuSliderStrings.youHave + GlobalVariables.orders.length + localeStrings.menuSliderStrings.itemInList;
    ...
    showCheckIn() {
        AsyncStorage.getItem('checkIn').then((value) => {
            ...
            AsyncStorage.getItem('locationDiscount').then((lDiscount) => {
        ...
        if (this.params.showWelcome) {
            setTimeout(() => {
                Alert.alert(
                    ...
                    [{
                        text: localeStrings.menuSliderStrings.thankYou, onPress: () => console.log("Ok")
                    }
                    ],
                    { cancelable: false }
                );
    ...
    addCoupon(coupon) {
        ...
        let OrderRequest = [];
        for (let i = 0; i < GlobalVariables.orders.length; i++) {

            let orderItem = {
                productId: GlobalVariables.orders[i].info.id,
                variationName: null,
                quantity: GlobalVariables.orders[i].quantity,
                dressing: GlobalVariables.orders[i].dressing,
                extraDressing: GlobalVariables.orders[i].extraDressing,
                specialInstructions: GlobalVariables.orders[i].specialInstructions
            };
            OrderRequest.push(orderItem);
        }
        this.props.navigation.navigate("Payment", {
            screen: "Payment",
            User: GlobalVariables.userId.value,
    ...
    componentWillMount() {
        ...
        this.keyboardDidShowListener = Keyboard.addListener('keyboardDidShow', (e) => {
            that.setState({ toScroll: false, scrollMarginTop: e.endCoordinates.height });
        });
    ...
    renderMainCategories() {
        return (
            <View style={{
                flexDirection: "row", paddingLeft: 5, paddingRight: 5, flex: 1
    ...
    renderSubCategory() {
        ...
        if (this.state.indexMainCategory === 0) {
            ...
            return (
                <View style={{ flexDirection: "row", paddingLeft: 5, paddingRight: 5, flex: 1, width: height > width ? height : width }}>
                    {this.state.subCategoryItems.map((items, index) => (
                        <View key={index} style={{ marginLeft: 15, justifyContent: "flex-end" }}>
    ...
    getMainCategories() {
        try {
            fetch(GlobalVariables._URL + "/categories/main/" + (this.params.barId ? this.params.barId : GlobalVariables.restId.value) + "/" + GlobalVariables.userLanguage.value, {
                method: "GET",
                headers: new Headers({
                    'Accept': 'application/json',
                    'Content-Type': 'application/x-www-form-urlencoded'
    ...
    getSubCategories(idSubCategory) {
        try {
            ...
            fetch(GlobalVariables._URL + "/categories/subcateg/" + idSubCategory + "/" + BarId + "/" + GlobalVariables.userLanguage.value, {
                method: "GET",
                headers: new Headers({
                    'Accept': 'application/json',
                    'Content-Type': 'application/x-www-form-urlencoded'
    ...
    render() {
        if (GlobalVariables.orders.length > 0) {
            ...
            this.state.totalOrders = localeStrings.menuSliderStrings.youHave + ' ' + GlobalVariables.orders.length + ' ' + localeStrings.menuSliderStrings.itemInList;
        }
                ...
                >
                    <View style={PageStyles.modalFirstView}>
                        <View style={PageStyles.modalSecondView}>
                            ...
                            <Image source={require("../../res/images/coupon1.png")}
                                resizeMode='cover'
                                style={PageStyles.modalImage}>
                            </Image>
...
class MyListItem extends React.PureComponent {
    ...
    render() {
        ...
        AsyncStorage.getItem('checkIn').then((checkedIn) => {
            (checkedIn === "true") ? this.setState({ isCheckIn: true }) : this.setState({ isCheckIn: false });
        }).done();
        return (
            <TouchableOpacity
                onPress={() => {
                ...
                <View style={{
                    ...
                    <View style={{ flexDirection: "column", width: "55%", marginBottom: 4 }}>
                        ...
                        {I18nManager.isRTL ? <Text style={{
                            ...
                            marginTop: 4,
                            textAlign: "left"
                        }}>₪ {this.props.itemPrice}</Text> : GlobalVariables.userLanguage.value === "he-IL" ?
                                <Text style={{
                                    color: "#4A4A4A",
                                    fontSize: 14,
                                    fontFamily: "Helvetica",
                    ...
                    <View style={{
                        ...
                        <Image style={{ resizeMode: "contain", width: 85, height: 87 }}
                            source={{ uri: this.props.itemImage }} />

                        {
                            ((this.state.isCheckIn === true) && (savedIDBar == this.params.barId)) ? (
                                <TouchableOpacity onPress={() => {
                                    this.addItemInMenu();
                                    this.props.navigation.navigate("MenuSlider", { screen: "MenuSlider" })
                                }}>
                                    <View style={{ alignItems: 'center', justifyContent: 'center', borderRadius: 50, marginLeft: 15, marginRight: 4 }}>
                                        <Image style={{
                                            width: 22,
                                            height: 22
                                        }}
                    ...
                    <Modal
                        ...
                        style={styles.modal}
                        visible={
                            this.state.IsCheckinModalVisible
                        }>
                        <View
                            ...
                            <View
                                ...
                                <View
                                    style={PageStyles.modalCenterText}>
                                    <Text style={{
                                        fontSize: 18,
                                        color: "gray",
                                        fontWeight: "bold"
                                ...
                                <View style={styles.container}>
                                    <TouchableOpacity style={styles.CenterText}
                                        ...
                                        <Text
                                            style={styles.textStyle}>{localeStrings.barLocationStrings.later}</Text>
                                    </TouchableOpacity>
                                    <View
                                        style={{
                                            borderLeftWidth: 1,
                                    ...
                                    <TouchableOpacity style={styles.CenterText}
                                        ...
                                        <Text
                                            style={styles.textStyle}>{localeStrings.barLocationStrings.checkIn}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </View>
...
class BackMenu extends React.Component {
    ...
    quantityAdd(index) {
        ...
        this.state.Order[index].price = this.state.Order[index].rawPrice * numberAdd;
    ...
    quantityMinus(index) {
        ...
        if (this.state.Order[index].quantity !== 1) {
            ...
            this.state.Order[index].price = this.state.Order[index].rawPrice * numberMinus;
    ...
    TotalAmount(reset) {
        ...
        if (this.state.Order.length > 0) {
            ...
            this.state.vat = (this.state.subTotal * (this.state.Order[0].info.taxPercent / 100)); // Quantity * taxAmount
            amountWithTax = this.state.subTotal;
            this.state.LocationDiscount = (this.state.subTotal * (GlobalVariables.locationDiscount.value / 100)).toFixed(2);
        ...
        return (
            <View style={{ flexDirection: "row" }}>
                <View style={{ flexDirection: "row", width: "50%", marginBottom: 220, justifyContent: "flex-start" }}>
                    <View style={{
                        flexDirection: "column",
                        marginLeft: 15,
                    }}>
                        ...
                        }}>{localeStrings.menuSliderStrings.subTotal}</Text>
                        <Text style={{
                            fontFamily: "Helvetica",
                            fontSize: 15,
                            // textAlign:I18nManager.isRTL? "left":"right",
                            marginTop: 8
                        }}>{localeStrings.menuSliderStrings.tax} ({GlobalVariables.taxRate.value}%)</Text>
                        {<Text style={{
                            fontFamily: "Helvetica",
                            fontSize: 15,
                            //  textAlign:I18nManager.isRTL? "left":"right",
                            marginTop: 8
                        }}>{localeStrings.menuSliderStrings.discount} ({GlobalVariables.locationDiscount.value}%)</Text>}
                        <Text
                            style={{
                                fontFamily: "Helvetica",
                                fontSize: 15,
                                marginTop: 15,
                                // textAlign:I18nManager.isRTL? "left":"right"
                            }}>{localeStrings.menuSliderStrings.total}</Text>
                    </View>
                </View>
                <View style={{ flexDirection: "row", width: "50%", marginBottom: 220, justifyContent: "flex-end" }}>
                    <View style={{
                        flexDirection: "column",
                        marginRight: 35,
                        ...
                        {I18nManager.isRTL ? <Text style={{ fontFamily: "Helvetica", fontSize: 15, marginTop: 8, marginRight: -5 }}>₪
                            {this.state.vat.toFixed(2)}</Text> : <Text style={{ fontFamily: "Helvetica", fontSize: 15, marginTop: 8 }}>
                                {this.state.vat.toFixed(2)}
                                {GlobalVariables.restCurrencyCode.value}</Text>}
                        {I18nManager.isRTL ? <Text style={{ fontFamily: "Helvetica", fontSize: 15, marginTop: 8, marginRight: -5 }}>₪
                            -{this.state.LocationDiscount}</Text> : <Text style={{ fontFamily: "Helvetica", fontSize: 15, marginTop: 8 }}> -{this.state.LocationDiscount}
                                {GlobalVariables.restCurrencyCode.value}</Text>}
                        {I18nManager.isRTL ? <Text style={{
                            fontFamily: "Helvetica",
                            fontSize: 15,
                        ...
                        }}>₪ {this.state.Total.toFixed(2)}</Text> : <Text style={{
                            fontFamily: "Helvetica",
                            fontSize: 15,
                            marginTop: 15,
                            fontWeight: "bold", marginRight: -5
                        }}>{this.state.Total.toFixed(2)} {GlobalVariables.restCurrencyCode.value}</Text>}
                    </View>
                </View>
            </View>
                        ...
                        return (
                            <View key={index} style={{
                                ...
                                <View style={{ flexDirection: "row", width: "100%" }}>
                                    <Image style={{ resizeMode: "contain", width: 80, height: 130 }}
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{ flexDirection: "column" }}>
                                            <Text style={{
                                                ...
                                                fontWeight: "bold",
                                                fontSize: 15
                                            }}>{items.info.name}</Text>
                                            <Text
                                                style={{
                                                    color: "#4A4A4A",
                                                    marginTop: 5,
                                                    fontSize: 14
                                                }}>{items.info.description}</Text>
                                        </View>

                                        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                            {!items.dressing && !items.extraDressing ? <View style={{
                                                flexDirection: "row",
                                                height: 45,
                                                justifyContent: "center",
                                                alignItems: "center"
                                                ...
                                                <TouchableOpacity onPress={() => this.quantityMinus(index)} style={{
                                                    height: 45,
                                                    width: 45,
                                                    justifyContent: "center",
                                                    alignItems: "center"
                                                ...
                                                <Text style={{ marginLeft: 8, marginRight: 8 }}>{items.quantity}</Text>

                                                <TouchableOpacity onPress={() => this.quantityAdd(index)} style={{
                                                    height: 45,
                                                    width: 45,
                                                    justifyContent: "center",
                                                    alignItems: "center"
                                            ...
                                            {I18nManager.isRTL ? <Text style={{
                                                color: "black",
                                                marginTop: 10,
                                            }}>₪ {items.price.toFixed(2)} {GlobalVariables.restCurrencyCode.value}</Text> :
                                                <Text style={{
                                                    color: "black",
                                                    marginTop: 10,
                                                }}>{items.price.toFixed(2)} {GlobalVariables.restCurrencyCode.value}</Text>}
                                ...
                                <View style={{ borderColor: "#4A4A4A", borderWidth: 0.5, height: 1 }} />
                            </View>
                        )
